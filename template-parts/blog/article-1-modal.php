<!-- Article 1 Modal -->
<div id="article-1" class="fixed inset-0 bg-black/80 z-[9999] hidden items-center justify-center p-4">
    <div class="bg-white rounded-2xl w-full max-w-3xl max-h-[85vh] overflow-hidden relative z-10 flex flex-col">
        <div class="relative h-64 shrink-0">
            <img src="<?php echo esc_url(get_theme_file_uri('assets/img/garde-chat-domicile-sans-stress-vaud.webp')); ?>" class="w-full h-full object-cover" alt="Chat détendu pendant garde à domicile" loading="lazy">
            <button onclick="document.getElementById('article-1').classList.add('hidden')" class="absolute top-4 right-4 bg-white/90 p-3 rounded-full hover:bg-white text-gray-800 transition shadow-sm font-bold" aria-label="Fermer"><i class="fas fa-times text-xl"></i></button>
        </div>
        <div class="p-8 overflow-y-auto custom-scrollbar bg-white">
            <article class="blog-article prose max-w-none text-gray-600">
                <header class="mb-6 border-b border-gray-100 pb-6">
                    ...
                    <h2 class="text-3xl font-title font-bold text-brand-text mb-4 sm:text-2xl">Pourquoi la garde à domicile réduit-elle le stress de votre chat&nbsp;?</h2>
                ...
                <div>
                    ...
                    <div class="bg-brand-cream p-6 rounded-xl mt-8 border border-brand-purple/20">
                        ...
                        <a href="#contact" onclick="document.getElementById('article-1').classList.add('hidden')" class="inline-block mt-4 text-brand-purple font-bold underline hover:text-brand-purple-dark">Réserver ma garde →</a>
                    </div>
                </div>
            </article>
        </div>
